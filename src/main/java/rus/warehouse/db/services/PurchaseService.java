...
public class PurchaseService {
    ...
    private PagedDataDto<PurchaseDTO> convertPageToDTO(Page<Purchase> purchaseList){
        ...
        for (Purchase purchase : purchaseList) {
            newList.add(new PurchaseDTO(purchase.getId(), purchase.getDate(), purchase.getPrice(), purchase.getCostWithVAT(), purchase.getCompany().getName()));
        }
    ...
    public Purchase createPurchase(GetPurchaseDTO getPurchaseDTO){
        Purchase purchase = new Purchase(getPurchaseDTO.getDate(), getPurchaseDTO.getPrice(), getPurchaseDTO.getCostWithVAT(),
                getPurchaseDTO.getCompany());
        purchase = purchaseRepository.save(purchase);

        for (PurchaseProduct prod:
             getPurchaseDTO.getProductList()) {
            //System.out.println("ЦИКЛ АТКИНСОНА");
            Product product = productService.createProduct(prod);
            //System.out.println(product);
            purchaseListService.create(new Purchase_List(prod.getCount(), purchase, product));
        }
